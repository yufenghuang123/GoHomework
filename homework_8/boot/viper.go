...
const (
	configEnv = "ZHIHU_CONFIG_PATH"           //预定环境变量
	confiFile = "manifest/config/config.yaml" //预定义配置文件位置
)
...
func ViperSetup(path ...string) {
	...
	if len(path) != 0 {
		...
		//命令行
		flag.String(&configPath, "c", "", "set config path")
		flag.Parse()
